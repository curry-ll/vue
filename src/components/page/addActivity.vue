<template>
  <div class="addActivity">
    ...
    <div class="agreement">
      ...
      <span @click="agreement()">我已阅读并同意相关发布活动协议</span>
    </div>
...
export default {
  ...
  methods: {
    ...
    agreement () {
      this.$router.push({path: '/agreement', query: {type: 3}})
    },
    // 选择开始日期
    selectStart () {
    ...
    submit () {
      if (this.agree) {
        ...
        var _this = this
        if (this.start_time) {
          this.start_time = this.start_time.getTime() / 1000
        }
        if (this.end_time) {
          this.end_time = this.end_time.getTime() / 1000
        }
        let data = {
          title: this.title,
