<template>
  <div class="serviceTime">
    ...
    <div class="duration-detail-title">
      <div class="warpper">
        <h3>服务时长详情</h3>
      </div>
    </div>
    ...
    >
      <div class="detail" v-for="(item,index) in durationList" :key="index">
        <div class="detail-wrapper">
          <p>{{item.create_time}}{{item.content}}</p>
          <span>{{item.status + item.duration}}分</span>
        </div>
      </div>
    </mt-loadmore>
  </div>
    ...
    getData () {
      ...
      this.$http({
        ...
        if (data.data.flag === 100) {
          _this.durationList = data.data.data
    ...
    loadBottom () {
      ...
      this.$http({
        ...
        if (data.data.data.length > 0) {
          _this.durationList.push(...data.data.data)
          data.data.data.map(function (item, index) {
            item.create_time = userDate(item.create_time)
          })
        } else {
